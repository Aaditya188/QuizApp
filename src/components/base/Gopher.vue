...
export default {
    ...
    created() {
        // if the user set a reduced motion in this OS
        ...
        if (safeToAnimate) {
            ...
            requestAnimationFrame(this.doTheMove);
        }
    },
    methods: {
        updateMouseCoords(e) {
            this.coords.xPosition = e.clientX;
            this.coords.yPosition = e.clientY;
        },
        percentage(val, totalVal) {
        ...
        doTheMove() {
            const x =
                this.percentage(this.coords.xPosition, this.windowWidth) - 50;
            const y =
                this.percentage(this.coords.yPosition, this.windowHeight) - 50;

            //update the css
            const svg = document.getElementById("gopher");
